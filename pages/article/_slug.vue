        ...
        <div class="col-xs-12 col-md-8 offset-md-2">
          <form v-if="$store.getters.user" class="card comment-form">
            ...
            <div class="card-footer">
              ...
              />
              <button
                @click.stop.prevent="handlePostComment"
          ...
          <div>
            <div v-for="(comment, i) in comments" :key="i" class="card">
              ...
              <div class="card-footer">
                ...
                <span
                  v-if="
                    $store.getters.user && comment.author.username == $store.getters.user.username
                  "
                  class="mod-options"
                >
...
export default {
  ...
  methods: {
    ...
    getComments() {
      this.$axios.get(`/articles/${this.$route.params.slug}/comments`).then(res => {
        this.comments = res.data.comments;
        this.comments.forEach(
          c => (c.updatedAtDisplay = moment(c.updatedAt).format('ddd MMM D YYYY')),
        );
      });
    },
    ...
    handleDeleteComment(commentId) {
      this.$axios
        .delete(`/articles/${this.$route.params.slug}/comments/${commentId}`)
        .then(this.getComments);
    },
  },

  mounted() {
    this.$axios.get(`/articles/${this.$route.params.slug}`).then(res => {
