        ...
        <div class="row">
          <div class="col-xs-12 col-md-10 offset-md-1">
            ...
            <button
              v-if="
                $store.getters.user &&
                  $store.getters.user.username == $route.params.username
              "
          ...
          <div class="articles-toggle">
            <ul class="nav nav-pills outline-active">
              <li class="nav-item">
                <a
                  href=""
                  @click.stop.prevent="selectedTab = 'MY_ARTICLES'"
                  class="nav-link"
                  :class="{ active: selectedTab == 'MY_ARTICLES' }"
                  >My Articles</a
                >
              </li>
              <li class="nav-item">
                <a
                  href=""
                  @click.stop.prevent="selectedTab = 'FAVORITED_ARTICLES'"
                  class="nav-link"
                  :class="{ active: selectedTab == 'FAVORITED_ARTICLES' }"
...
export default {
  ...
  mounted() {
    this.$axios.get(`/profiles/${this.$route.params.username}`).then(res => {
