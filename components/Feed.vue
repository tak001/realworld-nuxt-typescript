<template>
  <div>
    <div v-for="(article, i) in articles" :key="i" class="article-preview">
      <div class="article-meta">
        ...
        <div class="pull-xs-right">
          <button
            @click.stop.prevent="handleFavoriteClick(article, $event)"
            class="btn btn-sm"
    ...
    <nav>
      <ul class="pagination">
        <li
          v-for="(pageNumber, i) in Math.ceil(articlesCount / 10)"
          :key="i"
          :class="{ 'page-item': true, active: pageNumber == currentPage }"
        >
          <a href="" class="page-link" @click.prevent.stop="currentPage = pageNumber">
            {{ pageNumber }}
          </a>
        </li>
      </ul>
  ...
  methods: {
    async getArticles() {
      ...
      res.articles.forEach(
        a => (a.updatedAtDisplay = moment(a.updatedAt).format('ddd MMM D YYYY')),
      );
